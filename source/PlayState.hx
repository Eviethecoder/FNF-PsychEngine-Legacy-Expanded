...
class PlayState extends MusicBeatState
{
	...
	var triviabutton:Ghostfacebutton;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'postal2':
				var postal:BGSprite = new BGSprite('bg/bg-postal2', -650, 600, 0.9, 0.9);
				postal.setGraphicSize(Std.int(postal.width * 1.1));
				postal.updateHitbox();
				add(postal);
		...
		healthBar = new HealthBar(0, FlxG.height * (!ClientPrefs.downScroll ? 0.89 : 0.11), 'healthBar', function() return health, 0, 2);
	...
	function eventPushed(event:EventNote) {
		switch(event.event) {
			...
			case 'trivia':
				triviabutton = new Ghostfacebutton (0,0,event.value1, event.value2);
				triviabutton.screenCenter(X);
				triviabutton.screenCenter(Y);
				eventdone = 'true';
				add(triviabutton);
	...
	override public function update(elapsed:Float)
	{
		...
		var num:Int = Math.floor(Math.max(Math.min(Math.floor(healthBar.percent / 20), 4), 0));
	...
	override function beatHit()
	{
		...
		if (curSong == 'the-night-he-came-home')
			healthdrain();

		if (curSong == 'one-good-scare')
			healthdrain();
