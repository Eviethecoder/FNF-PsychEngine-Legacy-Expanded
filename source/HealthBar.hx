...
class HealthBar extends FlxSpriteGroup
{
	...
	public var barOffset:FlxPoint = new FlxPoint(3, 3); //x and y offsett
	...
	public function new(x:Float, y:Float, image:String = 'healthBar', valueFunction:Void->Float = null, boundX:Float = 0, boundY:Float = 1)
	{
		...
		switch (image){
			default:
				barWidth = Std.int(bg.width - 6); //the variables that cause pain
			...
			case('healthBarhg'):
				barWidth = Std.int(bg.width - 6); //the variables that cause pain
				barHeight = Std.int(bg.height - 50);

			case('healthbarcool'):
				barWidth = Std.int(bg.width - 110); //the variables that cause pain
				barHeight = Std.int(bg.height - 33);
		...
		//for whoever tries to make custom hp bars. 
	...
	public function updateBar()
	{
		...
		switch (hpbar){
			...
			case('healthBarhg'):
				leftBar.setPosition(bg.x, bg.y + 7);
				rightBar.setPosition(bg.x, bg.y + 7);

			case('healthbarcool'):
				leftBar.setPosition(bg.x +50, bg.y +13);
				rightBar.setPosition(bg.x+50, bg.y +13);
